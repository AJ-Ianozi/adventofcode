...
procedure Day2 is
   ...
   function What_I_Need (Them : Action; Need : Outcomes) return Action is
      ...
      Rev_Outcome : constant Outcomes := (case Need is
                                          when Win => Lose,
                                          when Lose => Win,
                                          when Draw => Draw);
   begin
   ...
   function Set_Round_Day2_1 (Test : String) return Round
   is
      ...
      Me_A : constant Action := (case Me is
                               when 'X' => Rock,
                               when 'Y' => Paper,
                               when 'Z' => Scissors,
                               when others => raise Invalid_Character);
      Them_A : constant Action := (case Them is
                               when 'A' => Rock,
                               when 'B' => Paper,
                               when 'C' => Scissors,
                               when others => raise Invalid_Character);
      --  Play the round, looking up the result in the truth table.
      Result : constant Round := (Me => Me_A,
                                  Them => Them_A,
                                  Result => TruthTable (Me_A)(Them_A));
   begin
      return Result;
   end Set_Round_Day2_1;

   --  Plays the round for Day-2-2
   function Set_Round_Day2_2 (Test : String) return Round
   is
      --  Grab the values from the string.
      Split : constant Integer := Index (Test, " ");
      Them : constant Character := Trim (Test (Test'First .. Split), Both) (1);
      Me : constant Character := Trim (Test (Split .. Test'Last), Both) (1);
      --  Decodes the character, and sets what each one selected.
      Me_O : constant Outcomes := (case Me is
                               when 'X' => Lose,
                               when 'Y' => Draw,
                               when 'Z' => Win,
                               when others => raise Invalid_Character);
